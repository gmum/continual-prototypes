...
def main(argv=None):
    ...
    parser.add_argument(
        "--log",
        default=["disk", "tensorboard", "neptune"],
        type=str,
        choices=["disk", "tensorboard", "neptune"],
        help="Loggers used (disk, tensorboard, neptune) (default=%(default)s)",
        nargs="*",
        metavar="LOGGER",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num_classes", type=int, help="Number of classes in the whole dataset"
    )
    parser.add_argument(
        "--num_warm", type=int, help="Number of warm epochs for ProtoPNet"
    )
    parser.add_argument(
        "--push_at", type=int, help="Every how many epochs perform push"
    )
    parser.add_argument(
        "--num_push_tune", type=int, help="NUmber of finetuning epochs after push"
    )
    parser.add_argument(
        "--ppnet_eval", action="store_true", help="Use ProtoPNet evaluation"
    )
    parser.add_argument(
        "--ppnet_sim", type=str, help="similarity function", choices=["exp", "log"]
    )
    parser.add_argument(
        "--sep_weight", type=float, help="Value of sep loss", default=-0.08
    )
    parser.add_argument(
        "--sep_add_ons", action="store_true", help="Each head with different add_ons"
    )
    parser.add_argument(
        "--incorrect_weight", type=float, help="Value of sep loss", default=-0.5
    )
    parser.add_argument(
        "--incorrect_weight_btw_tasks",
        action="store_true",
        help="Incorrect weights between heads",
    )
    ...
    if "protopnet" in args.network:
        ...
        init_model = construct_PPNet(
            args.network.replace("protopnet_", ""),
            pretrained=True,
            img_size=224,
            prototype_shape=(
                int(args.proto_num_per_class * num_cls),
                args.proto_depth,
                1,
                1,
            ),
    ...
    if isinstance(init_model, PPNet) or isinstance(init_model, TesNet):
        net = LLL_Net_PPNet(
            init_model, remove_existing_head=not args.keep_existing_head
        )
    else:
    ...
    for t, (_, ncla) in enumerate(taskcla):
        ...
        # Test
        for u in range(t + 1):
            test_loss, acc_taw[t, u], acc_tag[t, u], ppnet_losses_tu = appr.eval(
                u, tst_loader[u]
            )
            ...
            for p_key in ppnet_losses_tu.keys():
                logger.log_scalar(
                    ...
                    name=p_key,
                    group="test",
                    value=ppnet_losses_tu[p_key],
                )

            if args.ppnet_eval:
                proto_act_fig, ppnet_inclass_fig, ppnet_outclass_fig = appr.ppnet_eval(
                    u, tst_loader[u]
                )
                logger.log_figure(name="proto_act", iter=u, figure=proto_act_fig)
                logger.log_figure(
                    name="ppnet_inclass_fig", iter=u, figure=ppnet_inclass_fig
                )
                logger.log_figure(
                    name="ppnet_outclass_fig", iter=u, figure=ppnet_outclass_fig
                )
