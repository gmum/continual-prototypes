...
class Logger(ExperimentLogger):
    ...
    def __init__(self, log_path, exp_name, begin_time=None):
        ...
        sys.stdout = FileOutputDuplicator(
            sys.stdout,
            os.path.join(self.exp_path, "stdout-{}.txt".format(self.begin_time_str)),
            "w",
        )
        sys.stderr = FileOutputDuplicator(
            sys.stderr,
            os.path.join(self.exp_path, "stderr-{}.txt".format(self.begin_time_str)),
            "w",
        )

        # Raw log file
        self.raw_log_file = open(
            os.path.join(self.exp_path, "raw_log-{}.txt".format(self.begin_time_str)),
            "a",
        )
    ...
    def log_figure(self, name, iter, figure, curtime=None):
        ...
        figure.savefig(
            os.path.join(
                ...
                "figures",
                "{}_{}-{}.png".format(
                    name, iter, curtime.strftime("%Y-%m-%d-%H-%M-%S")
                ),
            )
        )
        figure.savefig(
    ...
    def save_model(self, state_dict, task):
        torch.save(
            state_dict,
            os.path.join(self.exp_path, "models", "task{}.ckpt".format(task)),
        )
